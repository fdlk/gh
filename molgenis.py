...
class Session():
    ...
    def update_all(self, entity, entities):
        ...
        response = self.session.put(self.url + "v2/" + quote_plus(entity),
                                     headers=self._get_token_header_with_content_type(),
    ...
    def getv2(self, entity, q=None, attributes=None, expand=None, num=100, start=0, sortColumn=None, sortOrder=None):
        '''Retrieves entity rows from an entity repository.

        Args:
        entity -- fully qualified name of the entity
        q -- query in rsql form, see the MOLGENIS REST API v2 documentation for details
        attributes -- The list of attributes to retrieve
        expand -- the attributes to expand
        num -- the amount of entity rows to retrieve
        start -- the index of the first row to retrieve (zero indexed)
    ...
    def delete(self, entity, id):
        ...
        response = self.session.delete(self.url + "v1/" + quote_plus(entity) + "/" + quote_plus(id),
                                       headers=self._get_token_header())
        response.raise_for_status();
        return response;
    ...
    def get_entity_meta_data(self, entity):
        ...
        response = self.session.get(self.url + "v1/" + quote_plus(entity) + "/meta?expand=attributes",
                                    headers=self._get_token_header())
        response.raise_for_status();
        return response.json();
    ...
    def get_attribute_meta_data(self, entity, attribute):
        ...
        response = self.session.get(self.url + "v1/" + quote_plus(entity) + "/meta/" + quote_plus(attribute),
                                    headers=self._get_token_header())
        response.raise_for_status();
        return response.json();
